...
graph TD
    ...
    subgraph P1 [第一階段：基礎與預組裝]
        S1[基礎審查與打毛 Foundation Review/Chipping] --> S2[主鋼構門型架預組裝 Main Steel Pre-assembly]
    ...
    subgraph P2 [第二階段：主鋼構與外殼安裝]
        ...
        S6 --> S7[水平Support樑安裝 Bracing Beams]
    ...
    subgraph P4 [第四階段：SCR、汽鼓與輔助設備]
        S13 --> S16[汽鼓Support樑與汽鼓吊裝 Drum Beams & Drums]
...
| ---------------------------- | ------------------------------------------------------ | ---------------------------------------------- |
| **基礎審查 (Foundation Review)** | 提供第三方調查報告；核實地栓座標、標高與預留孔清潔度；完成接地工程。                     | **釋出主鋼構安裝作業**：確保基礎接觸區已完成打毛 (Chipping) 施工。      |
| **主鋼構門型架預組裝**                | 準備水平墊木（若在基礎上組裝需保護）；清點材料熱號(material heat no’s)與 OSD 報告。 | **釋出預組裝門型架吊裝**作業：在水平狀態完成橫向緩衝器Support安裝並通過最終檢查。 |
| **煙囪段預組裝 (Stack Sections)**  | 準備水平平台；在殼板標記 0/120/240 度參考點；搭建內外施工架。                   | **釋出煙囪現場零件、平台、儀器接頭與外部保溫安裝作業**。                 |
| **煙囪導風板預組裝 (Damper)**        | 準備水平平台並校平；移除供應商彩色標記的固定裝置。                              | **釋出導風板縱向焊道焊接作業**。                             |
| **基礎灌漿 (Grouting)**          | 主鋼構、二次鋼構或設備底座已就位；模板與混凝土面清潔完成。                          | **釋出後續養護作業：需視養護時間（通常約 5 天）。                    |

### 第二階段：主鋼構與外殼安裝

| 施工步驟                       | 準備工作與前提條件 (Preparations and preconditions)         | 釋出後續活動 (Release subsequent activities)  |
| -------------------------- | -------------------------------------------------- | --------------------------------------- |
| **主鋼構吊裝 (Main Steel)**     | 基礎釋出；墊片 (Shim stacks) 已校平且不突出；吊裝前完成緩衝器支撐Support預裝。 | **釋出水平Support樑與汽鼓Support樑安裝**。          |
| **主鋼構水平樑 (bracing beams)** | 門型架吊裝完成；測量立柱間距以確定補償墊片厚度。                           | **釋出散裝外殼板安裝**。                          |
| **外殼底板安裝 (Bottom Casing)** | 主鋼構對心完成；準備施工架與保護襯板用的膠合板 (Plywood)。                 | **釋出底板保溫/襯板、角鐵連接與側板安裝**。                |
| **外殼側板安裝 (Side Wall)**     | 主鋼構對心完成；材料清點與 OSD 調查。                              | **釋出保溫/襯板/現場拼接 field splices 與角鐵連接施工**。 |
| **內部保溫與襯板施工**              | 外殼面板焊接完成並清淨；保溫銷已焊接定位。                              | **釋出內部導風板與密封板安裝**。                      |

#### 8.1 主鋼構 (Main steel structure)

-   **安裝順序：** 底座加墊片調整高程 → 門型架吊裝 (附帶橫向緩衝器支撐Support) → 水平樑 (Bracing) 與汽鼓支撐樑安裝 → 接地連接。
-   **安裝細節：** 頂樑無吊耳，需使用**吊帶 (Slings)**；對心應從門型架內部執行，垂直度需符合公差圖。
-   **注意事項/要求：** 禁止用火切擴孔；對心時需放鬆臨時纜繩 (Guy wires)。

#### 8.3 外殼安裝 (Casing - Bottom, Side and Top)

-   **安裝順序：** 底板 (由前至後) → 角鐵連接 → 側板 → 模組吊裝後安裝前後側垂直面板 → 頂板。
-   **安裝細節：** ==外殼板與現場部件所有外部與內部銲縫均應為連續銲，不允許進行點銲==；利用膠合板 (Plywood) 保護底板襯板直至完工。
-   **注意事項/要求：** 外殼板現場部件可為填隙板filler plates、導板guide plates與加強板stiffener plates，焊接時需用焊接毯保護保溫棉；角落與現場拼接處之所有間隙必須使用填隙板 (6 毫米厚) 進行封閉 (密封銲)。
...
| **護肩結構 (Internal Shoulders)**  | 側壁外殼保溫襯板施工完成；核實材料清點。                       | **釋出模組鎖定 (Locking of bundles)**：確保間隙「a」正確。 |
| **模組鎖定 (Locking)**             | 內部護肩結構與繞接固定安裝完成。                           | **釋出內部管路安裝**：安裝臨時型鋼鎖定模組。                   |
...
-   **注意事項/要求：** 緩衝器與Support間需維持**自由間隙 (Gap A)**；導向板僅能焊在緩衝器上，禁止焊在Support上。

#### 8.7 模組安裝 (Module erection)

-   **安裝順序：** 接收 → 裝載至強背架 (Strong back) → 轉立直放 → 吊入鍋爐 → 連接懸吊樑 → 對心。
...
| **汽鼓吊裝與Support** | 模組安裝完成；汽鼓Support樑與 PTFE 滑動墊片安裝到位。 | **釋出汽鼓對心**。 |
